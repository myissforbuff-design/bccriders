...
export const Finances: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const ensureApprovedMembersHaveFeesAndMonthlyDues = (currentRecs: FinanceRecord[]) => {
      ...
      // Bail out when handed an empty list while approved members exist. That combination
      // means the finance logs have not loaded yet, not that the records are missing.
      // Reconciling against it would re-create every member's records at the configured
      // default amounts under the same document ids (rec_mf_<userId>, rec_md_*, rec_col_*),
      // overwriting whatever an admin had edited. It would also delete-sweep live records
      // in the cleanup passes below.
      if (updatedList.length === 0 && approvedUsers.length > 0) {
        return;
      }
        ...
        approvedUsers.forEach(u => {
          ...
          if (existsIdx === -1) {
            ...
            const newDueRec: FinanceRecord = {
              ...
              updatedAt: todayStr,
              // Insert-only on the server: never overwrite an admin-edited amount.
              autoGenerated: true,
            };
            ...
            } else if (!hasAnnualPromo && existingRec.status === 'Pending' && !existingRec.manualOverride && existingRec.amount !== due.amount) {
              hasNew = true;
      ...
      dynamicCols.forEach(col => {
        ...
        approvedUsers.forEach(u => {
          ...
          if (existsIdx === -1) {
            ...
            const existingRec = updatedList[existsIdx];
            if (existingRec.status === 'Pending' && !existingRec.manualOverride && existingRec.amount !== perMemberAmount) {
              hasNew = true;
    ...
    const handleUsersOrFinancesUpdated = () => {
      ...
      setUsers(freshUsers);
      // Only reconcile against a list that actually loaded. An empty read here (this
      // handler fires on every bcc_users_updated / bcc_finance_updated event, including
      // ones dispatched before the finance logs arrive) would otherwise be mistaken for
      // "no records exist" and regenerate every member's fee at the configured default.
      const latestRecs = loadFromSession<FinanceRecord[]>(LOCAL_STORAGE_REC_KEY, []);
      if (!Array.isArray(latestRecs) || latestRecs.length === 0) return;
      ensureApprovedMembersHaveFeesAndMonthlyDues(latestRecs);
    };
  ...
  const saveRecordsToStorage = (updatedRecs: FinanceRecord[]) => {
    ...
    // store.getFinanceRecords() reads the API cache before localStorage, and that cache
    // has no TTL. Without these it keeps serving the pre-edit list to the Dashboard,
    // rider profiles and exports until the next successful server fetch.
      ...
      async () => {
        ...
        if (editingRecord) {
          ...
          const updatedRecord: FinanceRecord = {
            ...
            updatedAt: todayStr,
            // Locks this record against every auto-generation / auto-normalization pass.
            manualOverride: true,
            autoGenerated: false,
          };
