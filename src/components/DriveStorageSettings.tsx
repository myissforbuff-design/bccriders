...
export const DriveStorageSettings: React.FC = () => {
  ...
  const handleMigrateExisting = async () => {
    if (!window.confirm('Scan MongoDB and upload all existing Base64 avatars and motorcycle photos to Google Shared Drive? This will replace Base64 strings in MongoDB with clean Google Drive URLs.')) {
      return;
    }
    setIsMigrating(true);
