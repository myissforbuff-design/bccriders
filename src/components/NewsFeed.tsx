...
export const NewsFeed: React.FC = () => {
  ...
  // Upload to Google Shared Drive via /api/drive/upload
  const handleMediaSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isImage && !isVideo) {
      alert('Please select an image or video file (JPG, PNG, WEBP, MP4, MOV, WEBM).');
      return;
    }
                  ...
                  <div className="flex items-center gap-1">
                    {(isAdmin || currentUser?.id === post.authorId) && (
                      <button
                        ...
                        className="p-1 rounded-md text-[#8a9695] hover:text-rose-600 hover:bg-rose-50 transition-colors cursor-pointer"
                        title="Delete Post"
                      >
              ...
              <form onSubmit={handleCreatePost} className="p-4 space-y-3.5 overflow-y-auto flex-1 text-xs">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 rounded-xl bg-[#f7f9f7] border border-[#e2ece2] text-xs font-semibold text-[#1b4332] focus:outline-none focus:border-[#2d6a4f]"
                  >
