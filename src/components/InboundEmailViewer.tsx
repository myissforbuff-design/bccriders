  ...
  const handleConfirmDeleteAll = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setEmails([]);
        setSelectedEmail(null);
        setDeleteSuccessMessage('All received emails have been deleted from inbox.');
        setTimeout(() => setDeleteSuccessMessage(null), 3500);
      } else {
        setDeleteSuccessMessage('Failed to clear inbox.');
        setTimeout(() => setDeleteSuccessMessage(null), 3000);
      }
    } catch (err) {
      console.error('Failed to delete all inbound emails:', err);
      setDeleteSuccessMessage('Failed to clear inbox due to network error.');
      setTimeout(() => setDeleteSuccessMessage(null), 3000);
    } finally {
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {isAdmin && emails.length > 0 && (
              <button
                ...
                title="Delete all received emails"
              >
      ...
      <AnimatePresence>
        ...
        {/* Admin Delete ALL Confirmation Modal */}
        {showDeleteAllModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 z-60">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 10 }}
            ...
            >
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-[#52605d] mt-0.5">
                    This will permanently clear all <strong>{emails.length}</strong> inbound email records from the database.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-rose-50/70 rounded-xl p-3 border border-rose-200 space-y-1 text-xs text-rose-900">
                ...
                <p className="text-[11px] text-rose-700 leading-relaxed">
                  All stored messages and sender history currently in the inbox will be completely purged.
                </p>
              </div>
