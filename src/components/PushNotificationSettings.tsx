...
export const PushNotificationSettings: React.FC<PushNotificationSettingsProps> = ({
  ...
  const [pushDivTab, setPushDivTab] = useState<'status' | 'channels' | 'preview' | 'simulator' | 'guide'>('status');

  const PUSH_SUB_TABS = [
    { id: 'status', label: 'Status', fullLabel: 'Master Status', icon: Bell },
    { id: 'channels', label: 'Channels', fullLabel: 'Channels & Alerts', icon: Radio },
    { id: 'preview', label: 'Preview', fullLabel: 'Banner Preview', icon: Smartphone },
    { id: 'simulator', label: 'Test', fullLabel: 'Test Simulator', icon: Send },
    { id: 'guide', label: 'Guide', fullLabel: 'Setup Guide', icon: Info },
  ] as const;
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-4xl mx-auto">
      {/* Sub-Tabs Button Group in Push Notifications (5 items: Status, Channels, Preview, Test, Guide) */}
      <div className="w-full py-0.5">
        <div
          role="group"
          aria-label="Push Notification Sub-Tabs"
          className="grid grid-cols-5 p-1 bg-[#eaefe9] rounded-xl border border-[#d6e2d7] w-full"
        >
          {PUSH_SUB_TABS.map((tab) => {
            const Icon = tab.icon;
            const isSelected = pushDivTab === tab.id;
            return (
              <button
                key={tab.id}
                type="button"
                onClick={() => setPushDivTab(tab.id as any)}
      ...
      {pushDivTab === 'status' && (
        <div className="bg-white rounded-2xl p-3.5 sm:p-5 md:p-6 border border-[#e2ece2] shadow-xs space-y-4 sm:space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
            <div className="flex items-center gap-2.5 sm:gap-3">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-[#d8f3dc] flex items-center justify-center text-[#1b4332] shrink-0">
                <Bell className="w-5 h-5 sm:w-6 sm:h-6 text-[#2d6a4f]" />
              </div>
              <div>
                <h3 className="font-extrabold text-sm sm:text-base text-[#1b4332]">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
              <div className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-full bg-[#f0f9f1] text-[#1b4332] font-bold text-[10px] sm:text-[11px] border border-[#a7d7b5]">
                <Radio className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#2d6a4f] animate-pulse" />
                <span>{deviceStats.count > 0 ? `${deviceStats.count} device${deviceStats.count === 1 ? '' : 's'} registered` : 'Cross-Device Ready'}</span>
              </div>
              {permission === 'granted' ? (
                <div className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-3.5 py-1 sm:py-1.5 rounded-full bg-[#d8f3dc] text-[#1b4332] font-extrabold text-[11px] sm:text-xs border border-[#74c69d]">
                ...
                <button
                  ...
                  disabled={loading}
                  className="px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl bg-[#1b4332] text-white font-extrabold text-[11px] sm:text-xs hover:bg-[#2d6a4f] transition-all flex items-center gap-1.5 sm:gap-2 shadow-xs cursor-pointer"
                >
                  {loading ? <OfficialDotSpinner size="sm" /> : <Bell className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
      ...
      {pushDivTab === 'channels' && (
        <div className="bg-white rounded-2xl p-3.5 sm:p-5 md:p-6 border border-[#e2ece2] shadow-xs space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
            ...
            <div className="p-3 sm:p-4 rounded-xl bg-[#f7f9f7] border border-[#e2ece2] flex items-start justify-between gap-2.5 sm:gap-3">
              <div className="flex items-start gap-2.5 sm:gap-3 min-w-0">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-[#d8f3dc] flex items-center justify-center text-[#1b4332] shrink-0 mt-0.5">
                  <DollarSign className="w-4 h-4 sm:w-5 sm:h-5 text-[#2d6a4f]" />
                </div>
              ...
              <input
                ...
                className="w-4 h-4 sm:w-5 sm:h-5 accent-[#1b4332] rounded cursor-pointer mt-1 shrink-0"
              />
            </div>
            ...
            <div className="p-3 sm:p-4 rounded-xl bg-[#f7f9f7] border border-[#e2ece2] flex items-start justify-between gap-2.5 sm:gap-3">
              <div className="flex items-start gap-2.5 sm:gap-3 min-w-0">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-[#d8f3dc] flex items-center justify-center text-[#1b4332] shrink-0 mt-0.5">
                  <UserCheck className="w-4 h-4 sm:w-5 sm:h-5 text-[#2d6a4f]" />
                </div>
                <div className="min-w-0">
                  <p className="text-xs sm:text-sm font-bold text-[#1b4332]">
                    Member Approvals
            ...
            <div className="p-3 sm:p-4 rounded-xl bg-[#f7f9f7] border border-[#e2ece2] flex items-start justify-between gap-2.5 sm:gap-3">
              <div className="flex items-start gap-2.5 sm:gap-3 min-w-0">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-[#d8f3dc] flex items-center justify-center text-[#1b4332] shrink-0 mt-0.5">
                  <Calendar className="w-4 h-4 sm:w-5 sm:h-5 text-[#2d6a4f]" />
                </div>
                <div className="min-w-0">
                  <p className="text-xs sm:text-sm font-bold text-[#1b4332]">
                    Activities & Rides
            ...
            <div className="p-3 sm:p-4 rounded-xl bg-[#f7f9f7] border border-[#e2ece2] flex items-start justify-between gap-2.5 sm:gap-3">
              <div className="flex items-start gap-2.5 sm:gap-3 min-w-0">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-[#d8f3dc] flex items-center justify-center text-[#1b4332] shrink-0 mt-0.5">
                  <Radio className="w-4 h-4 sm:w-5 sm:h-5 text-[#2d6a4f]" />
                </div>
                <div className="min-w-0">
                  <p className="text-xs sm:text-sm font-bold text-[#1b4332]">
                    Announcements & News
            ...
            {/* SOS & Emergency */}
            <div className="p-3 sm:p-4 rounded-xl bg-rose-50 border border-rose-200 flex items-start justify-between gap-2.5 sm:gap-3 md:col-span-2">
              <div className="flex items-start gap-2.5 sm:gap-3 min-w-0">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-rose-100 flex items-center justify-center text-rose-600 shrink-0 mt-0.5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10.5px] sm:text-xs text-rose-700 mt-0.5 leading-snug">
              ...
              <input
                ...
                className="w-4 h-4 sm:w-5 sm:h-5 accent-rose-600 rounded cursor-pointer mt-1 shrink-0"
              />
            </div>
          </div>
