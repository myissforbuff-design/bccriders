...
export class DataStoreService {
  ...
  recordMembershipFeePayment(approvedUser: User): void {
    ...
    // An empty local snapshot almost always means the finance logs have not finished
    // loading yet (fresh browser, cleared storage, or this runs before the fetch
    // resolves). Treating that as "no fee on file" regenerates the record at the
    // configured default and, because it reuses the rec_mf_<userId> document id,
    // wipes out an amount the admin edited. The server backfills genuinely missing
    // fee records on its own, so it is always safe to skip here.
    ...
    if (!exists) {
      ...
      const newRec = {
        ...
        updatedAt: todayStr,
        // Flags this as a backfill so the server inserts it only and never
        // overwrites an amount an admin already edited by hand.
        autoGenerated: true,
      };
