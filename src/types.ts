...
export interface FinanceRecord {
  ...
  createdAt?: string;
  /**
   * Set when an Admin/Treasurer saved this record by hand. Auto-generation and
   * auto-normalization routines must never overwrite a flagged record, since
   * they share the same document id (e.g. rec_mf_<userId>) and would otherwise
   * reset the amount back to the configured default.
   */
  manualOverride?: boolean;
  /**
   * Set on records produced by an automatic backfill/reconciliation pass. The
   * server uses this to insert-only ($setOnInsert) instead of overwriting.
   */
