...
app.use((req, res, next) => {
  ...
  res.setHeader(
    'Permissions-Policy',
    'camera=(self), microphone=(), geolocation=(self), publickey-credentials-get=*, publickey-credentials-create=*'
  );
...
// AUTH: Query whether a username has registered fingerprint/biometric credentials
app.post('/api/auth/check-biometric', async (req, res) => {
  const inputUsername = req.body?.username || req.body?.usernameOrEmail;
  if (!inputUsername) {
  ...
  if (database) {
    try {
      ...
      console.warn('MongoDB search biometric check notice:', err);
    }
  }
  ...
  const hasBiometrics = Boolean(
    matchedUser.biometricEnabled &&
    Array.isArray(matchedUser.biometricCredentialIds) &&
    matchedUser.biometricCredentialIds.length > 0
  );

  res.json({
    success: true,
    hasBiometrics,
    credentialIds: hasBiometrics ? matchedUser.biometricCredentialIds : [],
    userId: matchedUser.id || matchedUser._id?.toString(),
  });
});

// AUTH: Verify Biometric Credential and complete login (bypassing OTP)
app.post('/api/auth/verify-biometric-login', async (req, res) => {
  const { username, usernameOrEmail, credentialId, password } = req.body || {};
  const inputUser = username || usernameOrEmail;
  if (!inputUser) {
    return res.status(400).json({ error: 'Username is required' });
  }

  const normalizedUsername = String(inputUser).trim().toLowerCase();
  let matchedUser: any = null;
  ...
  if (database) {
    try {
      ...
      console.warn('MongoDB search biometric login notice:', err);
    }
  }
  ...
  if (matchedUser.approvalStatus === 'Pending') {
    return res.status(403).json({
      error: 'Registration Pending: Your member application is awaiting admin approval before sign-in.',
    });
  }

  // Validate that biometrics are active on this account
  const storedCredentials: string[] = Array.isArray(matchedUser.biometricCredentialIds)
    ? matchedUser.biometricCredentialIds
    : [];

  const isMatchedCredential = credentialId && storedCredentials.includes(credentialId);

  // If password was also provided, verify it, or if biometric credential matches stored passkey
  if (!isMatchedCredential && !matchedUser.biometricEnabled) {
    return res.status(401).json({ error: 'Fingerprint access is not enabled for this account.' });
  }

  const memberId = matchedUser.id || matchedUser._id?.toString();
  const token = generateSessionToken(memberId, matchedUser.role || 'user');

  const userPayload = {
    id: memberId,
    username: matchedUser.username || normalizedUsername,
    email: matchedUser.email || '',
    name: matchedUser.name || matchedUser.firstName || matchedUser.username || 'Club Member',
    firstName: matchedUser.firstName || '',
    lastName: matchedUser.lastName || '',
    ...
    biometricCredentialIds: storedCredentials,
  };

  console.log(`[Biometric Login] Fingerprint authenticated for user ${userPayload.username} (${memberId}). OTP bypassed.`);
  ...
  res.json({
    ...
    user: userPayload,
    token,
    message: 'Fingerprint authentication successful. Welcome back!',
  });
});

// AUTH: Register or Update Biometric Credentials for a User
app.post('/api/auth/register-biometric', async (req, res) => {
  const { userId, username, credentialId, enabled } = req.body || {};
  if (!userId && !username) {
  ...
  try {
    ...
    else if (username) query.username = { $regex: new RegExp(`^${username}$`, 'i') };

    const member = await database.collection('members').findOne(query);
    if (!member) {
      return res.status(404).json({ error: 'Member not found.' });
    }

    let existingCredentials: string[] = Array.isArray(member.biometricCredentialIds)
      ? member.biometricCredentialIds
      : [];

    if (credentialId && !existingCredentials.includes(credentialId)) {
    ...
    await database.collection('members').updateOne(
      query,
      {
        $set: {
          ...
          biometricCredentialIds: isEnabled ? existingCredentials : [],
          updatedAt: new Date().toISOString(),
        },
      }
    );
    ...
    res.json({
      success: true,
      biometricEnabled: isEnabled,
      credentialIds: isEnabled ? existingCredentials : [],
      message: isEnabled
        ? 'Fingerprint authentication enabled successfully. You can now use your fingerprint to log in without OTP!'
        : 'Fingerprint authentication disabled.',
    });
  } catch (err: any) {
    console.error('Error saving biometric credential:', err);
    res.status(500).json({ error: err.message || 'Failed to update biometric credentials.' });
  }
});
