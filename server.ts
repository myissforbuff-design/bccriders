...
app.post('/api/mongodb/financeLogs', async (req, res) => {
  ...
  try {
    // Auto-generated records (membership fee / monthly due / collection backfills) share
    // their document id with the record an admin may have already edited by hand
    // (e.g. rec_mf_<userId>). Insert them only - never let a backfill pass rewrite the
    // amount, status or paid date of a record that already exists.
    if (record.autoGenerated) {
      const result = await database.collection('financeLogs').updateOne(
        { id: record.id },
        { $setOnInsert: { ...record, updatedAt: new Date().toISOString() } },
        { upsert: true }
      );
      return res.json({
        success: true,
        ...
        message:
          result.upsertedCount === 0
            ? 'Existing finance record preserved; auto-generated write ignored.'
            : 'Finance record created in MongoDB "financeLogs" collection table.',
        result,
      });
    }
...
app.post('/api/mongodb/financeLogs/bulk', async (req, res) => {
  ...
  try {
    // Never let a bulk restore (which replays the browser's local snapshot) clobber a
    // record an admin edited by hand. Only a payload that is itself flagged as a manual
    // save is allowed to overwrite a manually-edited document.
    const protectedIds = new Set<string>(
      (
        await database
          .collection('financeLogs')
          .find({ manualOverride: true }, { projection: { id: 1 } })
          .toArray()
      ).map((d: any) => d.id)
    );

    const writable = records.filter(
      (rec: any) => rec && rec.id && (rec.manualOverride === true || !protectedIds.has(rec.id))
    );

    const bulkOps = writable.map((rec: any) => ({
      updateOne: {
        filter: { id: rec.id },
    ...
    res.json({ success: true, count: writable.length, skipped: records.length - writable.length });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
